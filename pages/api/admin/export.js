...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const r of rows) {
      lines.push([r.id, new Date(r.created).toISOString(), r.total_assets, r.liabilities, r.net_assets, r.zakaat, r.submitted_by].join(','));
    }
    const csv = lines.join('\n');
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="zakat_logs.csv"');
